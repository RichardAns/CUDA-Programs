...
int main(int argc,char * argv[])
{
	...
	else printf("usage stencil2d type(1/2 f or 3/4 d) nx|256 ny|256 nz|256 iter_h|10 iter_g|10000 save|0 conv|5000 threadsx|32 threadsy|16\n");
	return 1;
}
...
template <typename T> int main2(int argc,char *argv[])
{
	int ver3d =     (argc>1) ?  atoi(argv[1]) : 1;     // 1/2: float kernels 1 or 3, 3/4: double kernels 1 or 2 
	...
	int save     =  (argc>7) ?  atoi(argv[7]) : 0;     // save result?
	...
	while(iter_conv < iter_gpu){
		...
		else for(int k=0;k<conv/2;k++){  // ping pong buffers dev_a and dev_b
	...
	b = dev_a;
	char name[256]; sprintf(name,"stencil3d_%d.raw",ver3d%2);
	...
	if(ver3d <3)printf("final convergence diff %13.6e for type float \n",diff_final);
	else        printf("final convergence diff %13.6e for type double\n",diff_final);
